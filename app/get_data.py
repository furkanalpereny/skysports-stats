...
def get_seasons(league):
    ...
    gelenVeri = gelenVeri[0].find_all(
        'ul', attrs={'class': 'section-nav__group'})
    gelenVeri = gelenVeri[0].find_all(
        'li', attrs={'class': 'section-nav__item'})
...
def get_matches(season):
    ...
    # son 180
    try:
        script = gelenVeri[0].find_all(
            'script', attrs={'type': 'text/show-more'})[0].text
...
def get_stats(link):
    ...
    # MATCH DETAILS
    match_details = rawData.find_all("div", attrs={
                                     "class": "match-head__fixture-side match-head__fixture-side--wide-score"})

    for idx, item in enumerate(match_details):
        team_name = item.find_all(
            "abbr", attrs={"class": "swap-text--bp10"})[0]["title"]
        team_score = item.find_all(
            "span", attrs={"class": "match-head__score"})[0].text
    ...
    # STATISTICS
    match_stats = rawData.find_all(
        "div", attrs={"class": "match-stats callfn"})
    match_stats_item = match_stats[0].find_all(
        "div", attrs={"class": "match-stats__item"})
    for idx, item in enumerate(match_stats_item):
        ...
        stat = item.find_all("span", attrs={
                             "class": "match-stats__bar", "data-role": "match-stat-home"})[0].text

        _dict[str(column_name + "_home")] = stat

        stat = item.find_all("span", attrs={
                             "class": "match-stats__bar", "data-role": "match-stat-away"})[0].text
